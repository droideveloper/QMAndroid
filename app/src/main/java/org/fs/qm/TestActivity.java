...
public class TestActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Variable var1 = new Variable.Builder().name("x1").index(1).coef(10d).bound(ISolver.Constants.LOWER).lhs(0d).rhs(0d).build();
        Variable var2 = new Variable.Builder().name("x2").index(2).coef(6d).bound(ISolver.Constants.LOWER).lhs(0d).rhs(0d).build();
        Variable var3 = new Variable.Builder().name("x3").index(3).coef(4d).bound(ISolver.Constants.LOWER).lhs(0d).rhs(0d).build();

        //MAX z = 10x1 + 6x2 + 4x3
        FnObjective obj = new FnObjective.Builder()
                            .name("simplex")
                            .type(Objective.MAXIMIZE)
                            .vars(Arrays.asList(var1, var2, var3))
                            .build();
        ...
        Variable   con1Var3 = new Variable.Builder().name("x3").index(3).coef(1d).build();
        Constraint con1 = new Constraint.Builder().name("p").index(1).lhs(0d).rhs(100d).bound(ISolver.Constants.UPPER).vars(Arrays.asList(con1Var1, con1Var2, con1Var3)).build();
        ...
        Variable   con2Var3 = new Variable.Builder().name("x3").index(3).coef(5d).build();
        Constraint con2 = new Constraint.Builder().name("q").index(2).lhs(0d).rhs(600d).bound(ISolver.Constants.UPPER).vars(Arrays.asList(con2Var1, con2Var2, con2Var3)).build();
        ...
        Variable   con3Var3 = new Variable.Builder().name("x3").index(3).coef(6d).build();
        Constraint con3 = new Constraint.Builder().name("r").index(3).lhs(0d).rhs(300d).bound(ISolver.Constants.UPPER).vars(Arrays.asList(con3Var1, con3Var2, con3Var3)).build();
